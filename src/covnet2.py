...
#  = os.path.join(OUTPUT_PATH, 'model_log')
...
def myParseArgs():
  ...
  parser.add_argument(
    ...
    type=str,
    choices=['HILL-CMDC', 'SUNIWARD-CMDC', 'UNet'],
    #required=True
    default='UNet'    # 改SUNIWARD-CMDC为UNet
  )
...
class HPF(nn.Module):
  def __init__(self):
    ...
    all_hpf_list_5x5 = np.array([all_hpf_list_5x5,all_hpf_list_5x5])
    hpf_weight = nn.Parameter(torch.Tensor(all_hpf_list_5x5).view(30, 2, 5, 5), requires_grad=False)


    self.hpf = nn.Conv2d(2, 30, kernel_size=5, padding=2, bias=False)
...
class MyDataset(Dataset):
    def __init__(self, index_path, BOSSBASE_COVER_DIR, BOSSBASE_STEGO_DIR, transform=None):
        ...
        # self.bossbase_cover_path = BOSSBASE_COVER_DIR + '/{}.tif'   # 修改ppm为tif
        # self.bossbase_stego_path = BOSSBASE_STEGO_DIR + '/{}.tif'
    ...
    def __getitem__(self, idx):
        ...
        # cover_path = self.bossbase_cover_path.format('%05d' % file_index)    # '%05d' % index[i]
        cover_path = self.bossbase_cover_path.format(file_index)    # '%05d' % index[i]
...
def main(args):
  ...
  STEGANOGRAPHY = args.STEGANOGRAPHY      # UNet
  EMBEDDING_RATE = args.EMBEDDING_RATE    # 0.4 bpc

  # BOSSBASE_COVER_DIR = '/home/weikangkang/datasets/ALASKA20000_Spatial1/ALASKA_v2_TIFF_256_COLOR_RANDOM2'
  # BOSSBASE_STEGO_DIR = '/data/wkk/ALASKA20000_Spatial1/ALASKA_v2_TIFF_256_COLOR_SUNIWARD-CMDC_0.2'
  BOSSBASE_COVER_DIR = '/data/zsx/ALASKA_v2_TIFF_256_COLOR_2w_01'
  BOSSBASE_STEGO_DIR = '/data/zsx/ALASKA_v2_TIFF_256_COLOR_stegos_2w_01_0.4'
  # 数据划分
  ...
  LOG_NAME = '{}-{}-{}-model_log.log'.format(STEGANOGRAPHY, EMBEDDING_RATE, DATASET_INDEX)
  ...
  # 程序中断可以加一条倒入参数
  # statePath='./CovNet/' + PARAMS_NAME1

  # print("?? PARAMS_PATH: ", PARAMS_PATH)
  # print("?? PARAMS_PATH1: ", PARAMS_PATH1)
