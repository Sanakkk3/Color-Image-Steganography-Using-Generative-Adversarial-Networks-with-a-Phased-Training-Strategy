...
from ResUNet1DB_lambda import UNet
import os
...
def myParseArgs():
    ...
    parser.add_argument('--netG', default='./asym/netG_epoch_G7_cov_wise_DDLloss_withW_RB_pub_72.pth', help="path to netG1 (to continue training)")
    parser.add_argument('--config', default='ALASKA_stegos_4w_0.4_G7_cov_wise_DDLloss_withW_RB_pub', help="train result")

    parser.add_argument('--datacover', help='path to dataset',default='/dataroot') #  traning set (40,000 images)
    parser.add_argument('--datastegoC1', help='path to dataset',default='/data/ALASKA_stegos_4w_0.4_G7_cov_wise_DDLloss_withW_R_pub/')
    parser.add_argument('--indexpath', help='path to index',default='./index_list/alaska_train_4w.npy')
...
def STC(batch_x, p_plus, p_minus, index):

    rho_plus = torch.log(1/p_plus-2)
    rho_minus = torch.log(1/p_minus-2)

    ######################## STC embedding #############
    message_length = IMAGESIZE*IMAGESIZE*PAYLOAD
    ...
    rho_plus[batch_x==255] = WETCOST
    rho_minus[batch_x==0] = WETCOST

    batch_y = embedding_simulator(batch_x, rho_plus, rho_minus, message_length , index)
...
class ALADataset256(data.Dataset):
    def __init__(self, root, c1root, index_path, transforms = None):
    ...
    def __getitem__(self, index):
        ...
        sample = {'data': data, 'rand_ud': rand_ud, 'label': label, 'index': file_index, 'stegoC1': stegoC1}
        if self.transforms:
...
class ALAToTensor():
    def __call__(self, sample):
        data, rand_ud, label, index, stegoC1= sample['data'], sample['rand_ud'], sample['label'], sample['index'], sample['stegoC1']
...
def main():
    ...
    dataset = ALADataset256(args.datacover, args.datastegoC1, args.indexpath, transforms=transform)
    ...
    with torch.no_grad():
        for i,sample in enumerate(dataloader,0):
            ...
            cover, rand_ud, label, index, stegoC1= sample['data'], sample['rand_ud'], sample['label'], sample['index'], sample['stegoC1']
            cover, n, label, stegoC1= cover.cuda(), rand_ud.cuda(), label.cuda(), stegoC1.cuda()

            # learn the probas
            input = torch.cat([stegoC1, cover], dim=1)
            p_p, p_m, lamda = netG(input)
            
            p_plus = p_p/2.0 + 1e-5
            p_minus = p_m/2.0 + 1e-5

            stego = STC(cover, p_plus, p_minus, index)
            m = stego - cover
