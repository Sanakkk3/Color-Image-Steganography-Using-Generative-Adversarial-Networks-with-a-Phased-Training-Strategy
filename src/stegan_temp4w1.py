...
def myParseArgs():
    ...
    parser.add_argument('--netG', default='./asym/netG_epoch_G7_cov_wise_DDLloss_withW_R_pub_72.pth', help="path to netG1 (to continue training)")
    parser.add_argument('--config', default='ALASKA_stegos_4w_0.4_G7_cov_wise_DDLloss_withW_R_pub', help="train result")
...
class ALADataset256(data.Dataset):
    def __init__(self, root, index_path, transforms = None):
    ...
    def __getitem__(self, index):
        ...
        sample = {'data': data, 'rand_ud': rand_ud, 'label': label, 'index': file_index}
        if self.transforms:
...
def main():
    ...
    try:
        stego_path = os.path.join(args.root)
    ...
    dataset = ALADataset256(args.datacover, args.indexpath, transforms=transform)
    ...
    with torch.no_grad():
        for i,sample in enumerate(dataloader,0):
            ...
            cover, rand_ud, label, index= sample['data'], sample['rand_ud'], sample['label'], sample['index']
            cover, n, label = cover.cuda(), rand_ud.cuda(), label.cuda()

            # learn the probas
            
            p_p, p_m, lamda = netG(cover)
